...
class TemporalPathAgg(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, num_relation, query_input_dim, message_func="distmult",
                 aggregate_func="pna", layer_norm=False, activation="relu", time_encoding=True,
    ...
    def forward(self, input, query, initial_stat, edge_index, edge_type, edge_time, size, all_graph_min_times,edge_weight=None):
        """
        ...
        output = self.propagate(input=input, relation=relation, initial_stat=initial_stat, edge_index=edge_index,
                                edge_type=edge_type, edge_time=edge_time,min_times=all_graph_min_times, size=size, edge_weight=edge_weight)
        return output
    ...
    def aggregate(self, input, edge_weight, index, dim_size):
        ...
        shape = [1] * input.ndim
        shape[self.node_dim] = -1  #node_dim
