...
class RotaryPositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        参数:
            x: 输入张量，形状为 [B, H, seq_len, d_model]

        返回:
            经过 RoPE 编码后的张量，与输入同形状
        """
        bs, seq_len, dim = x.shape
        # 获取对应位置的 sin/cos 编码 [1, seq_len, dim]
        sin_cos = self.emb[:seq_len].unsqueeze(0)  # 添加 batch 维度

        # 拆分成 sin 和 cos 部分 [1, seq_len, dim//2]
...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        """
        x: 输入张量，形状为 [B, num_windows, feature_dim]
        返回：加上位置编码后的张量
        """
