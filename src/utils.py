...
def create_working_directory(args):
    ...
    working_dir = os.path.join("../model", work_dir)
...
def split_subgraph(history_list, num_nodes, num_rels, windows_size):
    ...
    for i in range(num + (1 if first_length else 0)):
        ...
        history_graph_list.append(build_history_graph(num_nodes, num_rels, sub_history_list, history_list.device))
        start = end
...
def get_query_aware_subgraphs(history_graph_list, batch_entities, device, k_hop=1):
    ...
    for graph in history_graph_list:
        ...
        sampled_history.append(sampled_edges)
