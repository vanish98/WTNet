...
def load_index(input_path):
    ...
    with open(input_path) as f:
        for i, line in enumerate(f.readlines()):        # ids in relations.dict and entities.dict are sequentially ordered
...
for id in range(len(id2entity.keys())):
    ...
    if "(" in entity_str and ")" in entity_str:
        ...
        w1 = entity_str[:begin].strip()
        w2 = entity_str[begin+1: end]
        eid2wid.append([str(entity2id[entity_str]), "0", str(word2id[w1])])   # isA relation
        eid2wid.append([str(entity2id[entity_str]), "1", str(word2id[w2])])     # membership relation
    else:
